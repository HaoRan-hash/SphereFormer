...
def lasermix_aug(xyzi_sup, label_sup, inst_sup, xyzi_unsup, label_unsup, inst_unsup):
    'Generates one sample of data'   # for semantic-kitti
    # Mix sup and unsup for MixTeacher training
    global xyzil_mix_1
    xyzil_sup = np.concatenate((xyzi_sup,  label_sup, inst_sup), axis=1)  # [N, 6]
    xyzil_unsup = np.concatenate((xyzi_unsup,  label_unsup, inst_unsup), axis=1)  # [N, 6]
    ...
    if strategy == 'inc3phi1':
        ...
        xyzil_mix_1 = np.concatenate((xyzil_sup_p1, xyzil_unsup_p2, xyzil_sup_p3), axis=0)  # [N, 6]
        xyzil_mix_2 = np.concatenate((xyzil_unsup_p1, xyzil_sup_p2, xyzil_unsup_p3), axis=0)  # [N, 6]
        ...
        xyzil_mix_2 = np.concatenate(
            (xyzil_unsup_p1, xyzil_sup_p2, xyzil_unsup_p3, xyzil_sup_p4, xyzil_unsup_p5), axis=0)  # [N, 6]
    ...
    Xyzi = xyzil_mix_1[:, :-2]
    Label = xyzil_mix_1[:, -2].reshape((-1, 1))
    inst = xyzil_mix_1[:, -1].reshape((-1, 1))
    return Xyzi, Label, inst
