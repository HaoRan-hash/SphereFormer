...
from util.semantic_kitti_two_stream import SemanticKITTI
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    if args.arch == 'unet_spherical_transformer':
        from model.unet_spherical_transformer_two_stream import Semantic as Model
...
def train(train_loader, model, criterion, optimizer, epoch, scaler, scheduler, gpu):
    ...
    for i, batch_data in enumerate(train_loader):  # (n, 3), (n, c), (n), (b)
        ...
        sinput = spconv.SparseConvTensor(feat[:, :4], coord.int(), spatial_shape, args.batch_size)
        sinput_scene_flow = spconv.SparseConvTensor(feat[:, 4:], coord.int(), spatial_shape, args.batch_size)
        ...
        with torch.cuda.amp.autocast(enabled=use_amp):
            ...
            output = model(sinput, sinput_scene_flow, xyz, batch)   # output.shape = (n, num_class)
...
def validate(val_loader, model, criterion):
    ...
    for i, batch_data in enumerate(val_loader):
        ...
        sinput = spconv.SparseConvTensor(feat[:, :4], coord.int(), spatial_shape, args.batch_size_val)
        sinput_scene_flow = spconv.SparseConvTensor(feat[:, 4:], coord.int(), spatial_shape, args.batch_size)

        assert batch.shape[0] == feat.shape[0]
        
        with torch.no_grad():
            output = model(sinput, sinput_scene_flow, xyz, batch)
    ...
    for i, batch_data_list in enumerate(val_loader):
        ...
        with torch.no_grad():
            ...
            for batch_data in batch_data_list:
                ...
                output_i = model(sinput, sinput_scene_flow, xyz, batch)
