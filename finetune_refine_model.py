...
from model.refine_model import RefineModel
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    refine_model = RefineModel(based_argmax=False, sigmoid_threshold=0.8, num_class=19, embed_dim=32)
    ...
    if args.cvae_weight:
        if os.path.isfile(args.cvae_weight):
            if main_process():
                logger.info("=> loading cvae weight '{}'".format(args.cvae_weight))
            checkpoint = torch.load(args.cvae_weight, map_location='cpu')
            cvae_model.load_state_dict(checkpoint, strict=True)
            
            if main_process():
                logger.info("=> loaded cvae weight '{}'".format(args.cvae_weight))
        else:
            logger.info("=> no cvae weight found at '{}'".format(args.cvae_weight))
    cvae_model = cvae_model.decoder
    ...
    if args.distributed:
        ...
        if args.sync_bn:
            ...
            refine_model = torch.nn.SyncBatchNorm.convert_sync_batchnorm(refine_model).cuda()
        cvae_model = cvae_model.cuda()
        model = torch.nn.parallel.DistributedDataParallel(model, device_ids=[gpu])
        cvae_model = torch.nn.parallel.DistributedDataParallel(cvae_model, device_ids=[gpu])
        refine_model = torch.nn.parallel.DistributedDataParallel(refine_model, device_ids=[gpu])
    else:
    ...
    if args.val:
        if args.use_tta:
            ...
            validate(val_loader, model, None, None, criterion)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        loss_train, mIoU_train, mAcc_train, allAcc_train = train(train_loader, model, cvae_model, refine_model,
                                                                 criterion, optimizer, epoch, scaler, scheduler, gpu)
        if args.scheduler_update == 'epoch':
            scheduler.step()
        epoch_log = epoch + 1
        ...
        if args.evaluate and (epoch_log % args.eval_freq == 0):
            loss_val, mIoU_val, mAcc_val, allAcc_val = validate(val_loader, model, cvae_model, refine_model, criterion, use_refine=True)
        ...
        if (epoch_log % args.save_freq == 0) and main_process():
            ...
            filename = args.save_path + '/model/refine_model_last.pth'
            logger.info('Saving checkpoint to: ' + filename)
            if args.distributed:
                torch.save({'epoch': epoch_log, 'state_dict': refine_model.module.state_dict(), 'optimizer': optimizer.state_dict(),
                            'scheduler': scheduler.state_dict(), 'best_iou': best_iou, 'is_best': is_best}, filename)
            else:
                torch.save({'epoch': epoch_log, 'state_dict': refine_model.state_dict(), 'optimizer': optimizer.state_dict(),
                            'scheduler': scheduler.state_dict(), 'best_iou': best_iou, 'is_best': is_best}, filename)
            if is_best:
                shutil.copyfile(filename, args.save_path + '/model/refine_model_best.pth')
...
def train(train_loader, model, cvae_model, refine_model, criterion, optimizer, epoch, scaler, scheduler, gpu):
    ...
    for i, batch_data in enumerate(train_loader):  # (n, 3), (n, c), (n), (b)
        ...
        with torch.cuda.amp.autocast(enabled=use_amp):
            ...
            # refine feat_model
            output, bce_loss = refine_model(cvae_model, feat_model, feat[:, 4:], output, z_dim=16, 
                                  target=target, ignore_label=args.ignore_label)
            lovasz_loss = lovasz_softmax(output.softmax(dim=-1), target, ignore=args.ignore_label)
            # debug
            # print(f'rank {torch.distributed.get_rank()}, bce loss {bce_loss}')
            ...
            if loss_name == 'focal_loss':
                loss = focal_loss(output, target, criterion.weight, args.ignore_label, args.loss_gamma) + bce_loss + 3 * lovasz_loss
            elif loss_name == 'ce_loss':
                loss = criterion(output, target) + bce_loss + 3 * lovasz_loss
            else:
...
def validate(val_loader, model, cvae_model, refine_model, criterion, use_refine=False):
    if main_process():
    ...
    for i, batch_data in enumerate(val_loader):
        ...
        with torch.no_grad():
            feat_model, output = model(sinput, xyz, batch, return_feat=True)
            
            # refine feat_model
            if use_refine:
                output = refine_model(cvae_model, feat_model, feat[:, 4:], output, z_dim=16)
...
def validate_tta(val_loader, model, cvae_model, refine_model, criterion, use_refine=False):
    if main_process():
        logger.info('>>>>>>>>>>>>>>>> Start Evaluation >>>>>>>>>>>>>>>>')
    ...
    for i, batch_data_list in enumerate(val_loader):
        ...
        with torch.no_grad():
            ...
            for batch_data in batch_data_list:
                ...
                feat_model, output_i = model(sinput, xyz, batch, return_feat=True)
                
                # refine feat_model
                if use_refine:
                    output_i = refine_model(cvae_model, feat_model, feat[:, 4:], output, z_dim=16)
...
def validate_distance(val_loader, model, cvae_model, refine_model, criterion, use_refine=False):
    if main_process():
        logger.info('>>>>>>>>>>>>>>>> Start Evaluation >>>>>>>>>>>>>>>>')
    batch_time = AverageMeter()
    data_time = AverageMeter()
