...
class UBlock(nn.Module):
    def __init__(self, nPlanes, 
        ...
        scene_flow_blocks = {'block{}'.format(i): block(nPlanes[0], nPlanes[0], norm_fn, indice_key='scene_flow{}'.format(indice_key_id)) for i in range(block_reps)}
        scene_flow_blocks = OrderedDict(blocks)
        self.scene_flow_blocks = spconv.SparseSequential(scene_flow_blocks)
        ...
        if indice_key_id in sphere_layers:
            ...
            self.scene_flow_transformer_block = SphereFormer(
                nPlanes[0], 
                num_heads, 
                window_size, 
                window_size_sphere, 
                quant_size, 
                quant_size_sphere,
                indice_key='scene_flow_sphereformer{}'.format(indice_key_id),
                rel_query=rel_query, 
                rel_key=rel_key, 
                rel_value=rel_value, 
                drop_path=drop_path[0],
        ...
        if len(nPlanes) > 1:
            ...
            self.scene_flow_conv = spconv.SparseSequential(
                norm_fn(nPlanes[0]),
                nn.ReLU(),
                spconv.SparseConv3d(nPlanes[0], nPlanes[1], kernel_size=2, stride=2, bias=False, indice_key='scene_flow_spconv{}'.format(indice_key_id), algo=ConvAlgo.Native)
            )
    ...
    def forward(self, inp, inp_scene_flow, xyz, batch):

        assert (inp.indices[:, 0] == batch).all()
        
        output = self.blocks(inp)
        output_scene_flow = self.scene_flow_blocks(inp_scene_flow)
        ...
        if self.indice_key_id in self.sphere_layers:
            if self.indice_key_id in self.grad_checkpoint_layers:   # 这里的if是为了节约显存
                ...
                def run_scene_flow(feats_, xyz_, batch_):
                    return self.scene_flow_transformer_block(feats_, xyz_, batch_)
                transformer_features = torch.utils.checkpoint.checkpoint(run, output.features, xyz, batch)
                transformer_features_scene_flow = torch.utils.checkpoint.checkpoint(run_scene_flow, output_scene_flow.features, xyz, batch)
            else:
                transformer_features = self.transformer_block(output.features, xyz, batch)
                transformer_features_scene_flow = self.scene_flow_transformer_block(output_scene_flow.features, xyz, batch)
            output = output.replace_feature(transformer_features)
            output_scene_flow = output_scene_flow.replace_feature(transformer_features_scene_flow)

        identity = spconv.SparseConvTensor(output.features + output_scene_flow.features, output.indices, output.spatial_shape, output.batch_size)

        if len(self.nPlanes) > 1:
            output_decoder = self.conv(output)
            output_scene_flow_decoder = self.scene_flow_conv(output_scene_flow)
            ...
            output_decoder = self.u(output_decoder, output_scene_flow_decoder, xyz_next, batch_next.long())
            output_decoder = self.deconv(output_decoder)
            output = output.replace_feature(torch.cat((identity.features, output_decoder.features), dim=1))
            output = self.blocks_tail(output)
        else:   # encoder的最后一个阶段
            output = output.replace_feature(output.features + output_scene_flow.features)
...
class Semantic(nn.Module):
    def __init__(self, 
        ...
        self.scene_flow_conv = spconv.SparseSequential(
            spconv.SubMConv3d(3, m, kernel_size=3, padding=1, bias=False, indice_key='scene_flow1')
        )
    ...
    def forward(self, input, input_scene_flow, xyz, batch):
        '''
        :param input_map: (N), int, cuda
        '''

        output = self.input_conv(input)   # type(output): SparseConvTensor
        output_scene_flow = self.scene_flow_conv(input_scene_flow)
        output = self.unet(output, output_scene_flow, xyz, batch)
