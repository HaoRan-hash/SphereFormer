...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    if args.data_name == 'nuscenes':
        train_data = nuScenes(args.data_root, 
            info_path_list=['nuscenes_seg_infos_1sweeps_train.pkl', 'nuscenes_seg_infos_1sweeps_val.pkl'], 
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (epoch_log % args.save_freq == 0) and main_process():
            ...
            torch.save({'epoch': epoch_log, 'state_dict': model.state_dict(), 'optimizer': optimizer.state_dict(),
                        'scheduler': scheduler.state_dict(), 'mIoU_train': mIoU_train}, filename)

    if main_process():
        writer.close()
        logger.info('==>Training done!\mIoU_train: %.3f' % (mIoU_train))
